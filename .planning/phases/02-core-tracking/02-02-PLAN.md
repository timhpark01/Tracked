...
must_haves:
  ...
  artifacts:
    ...
    - path: "src/features/profiles/hooks/useMyProfile.ts"
      ...
      min_lines: 15
    - path: "src/features/profiles/hooks/useProfile.ts"
      provides: "useProfile hook for viewing any user's profile"
      min_lines: 15
    ...
    - path: "app/(app)/profile/[userId].tsx"
      provides: "Other user's profile view screen"
      min_lines: 30
  key_links:
...
3. Create `src/features/profiles/hooks/useProfile.ts`:
   - Use `useQuery` with key `['profile', userId]`
   - Accept `userId: string` as parameter
   - Fetch profile using `getProfile(userId)`
   - `enabled: !!userId`
   - This hook is for viewing OTHER users' profiles (PROF-04)

4. Create `src/features/profiles/hooks/useUpdateProfile.ts`:
   - Use `useMutation`
...
5. Create `src/features/profiles/index.ts`:
   ```typescript
...
Files exist: `ls src/features/profiles/`
  ...
  <done>
Profile service exports getProfile, updateProfile, createProfile. Hooks useMyProfile, useProfile, and useUpdateProfile ready for screens.
  </done>
</task>
...
2. Create `app/(app)/profile.tsx` (view own profile screen):
...
4. Create `app/(app)/profile/[userId].tsx` (view other user's profile screen - PROF-04):
   - Get `userId` from route params via `useLocalSearchParams()`
   - Use `useProfile(userId)` to fetch the profile
   - Show loading state while fetching
   - Display avatar (Image or placeholder), username, bio (read-only)
   - Show "User not found" if profile doesn't exist
   - NO edit button (viewing someone else's profile)
   - NativeWind styling matching profile.tsx

5. Update `app/(app)/index.tsx`:
...
App starts: `npx expo start` (manual check)
  ...
  <done>
Profile tab shows user profile with avatar, username, bio. Edit screen allows updating all fields including avatar upload. [userId] screen allows viewing other users' profiles.
  </done>
</task>
...
3. `ls app/\(app\)/profile/` - profile screens exist including [userId].tsx
4. Manually test: Login -> Profile -> Edit -> Save -> Changes persist
5. Manually test: Navigate to /profile/{someUserId} -> Shows that user's profile (read-only)
</verification>

<success_criteria>
- Profile service with getProfile, updateProfile, createProfile functions
- useMyProfile hook returns current user's profile
- useProfile hook returns any user's profile by ID (for viewing others)
...
- [userId] screen shows another user's profile (read-only, no edit button)

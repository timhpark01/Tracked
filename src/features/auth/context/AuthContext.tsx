...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    // IMPORTANT: Don't await async operations here - it can cause deadlock with setSession
    const { data: { subscription } } = supabase.auth.onAuthStateChange((event, newSession) => {
      ...
      if (newUser) {
        // Set authenticated immediately, check profile in background (non-blocking)
        setStatus('authenticated')
        // Defer profile check and query reset to avoid deadlock
        setTimeout(() => {
